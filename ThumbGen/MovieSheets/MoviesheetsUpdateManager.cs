...
namespace ThumbGen
{
    ...
    public class MoviesheetsUpdateManager
    {
        ...
        //    public bool GenerateFolderJpg { get; set; }
        //    public bool GenerateThumbnail { get; set; }
        //    public bool UpdateIMDbRating { get; set; }
        ...
        //        try
        //        {
        //            Loggy.Logger.Debug("Worker thread start: " + Thread.CurrentThread.ManagedThreadId);
        //            MoviesheetsUpdateManager.ApplyNewTemplateEx(false, this.MovieFiles, this.Templates, this.GenerateFolderJpg, this.GenerateThumbnail, this.UpdateIMDbRating);
        //            Loggy.Logger.Debug("Worker thread stop: " + Thread.CurrentThread.ManagedThreadId);
        //        }
        //        finally
        //        {
        ...
        //static void MoviesheetsUpdateManager_MovieDone(object sender, MovieDoneEventArgs e)
        //{
        //    //lock (MoviesheetsUpdateManager.ProcessedCount)
        //    //{
        //    //    MoviesheetsUpdateManager.ProcessedCount = (int)MoviesheetsUpdateManager.ProcessedCount + 1;
        //    //}

        //    //FileManager.ShowAdorner(string.Format("Processing {0} ({1}/{2}) done ...", Path.GetFileName(e.MovieFilename), MoviesheetsUpdateManager.ProcessedCount, MoviesheetsUpdateManager.TotalCount), true);
        //    //Helpers.DoEvents();
        //}
        ...
        //public static void ApplyNewTemplate2(IList<FileInfo> movieFiles, List<TemplateItem> templates)
        //{
        //    bool generateFolderJpg = FileManager.Configuration.Options.AutogenerateFolderJpg;
        //    bool generateThumb = FileManager.Configuration.Options.AutogenerateThumbnail;
        //    bool updateRating = FileManager.Configuration.Options.UpdateIMDbRating;
        ...
        //    Loggy.Logger.Debug("Start creating sheets");
        //    int _CPUCount = Environment.ProcessorCount;
        //    if (_CPUCount > 4)
        //    {
        //        _CPUCount = 4;
        ...
        //    if ((!FileManager.Configuration.Options.EnableMultiCoreSupport && _CPUCount > 1) || _CPUCount == 1)
        //    {
        //        Loggy.Logger.Debug("Main thread start processing: " + Thread.CurrentThread.ManagedThreadId);
        //        MoviesheetsUpdateManager.ApplyNewTemplateEx(true, movieFiles, templates, generateFolderJpg, generateThumb, updateRating);
        //        Loggy.Logger.Debug("Main thread stop processing: " + Thread.CurrentThread.ManagedThreadId);
        //    }
        //    else
        //    {
        //        FileManager.ShowAdorner("Processing ...", true);
        //        Helpers.DoEvents();
        //        try
        //        {
        //            WaitHandle[] _handles = new WaitHandle[_CPUCount];

        //            MoviesheetsUpdateManager.MovieDone += new EventHandler<MovieDoneEventArgs>(MoviesheetsUpdateManager_MovieDone);
        //            MoviesheetsUpdateManager.TotalCount = movieFiles.Count;

        //            // more than one CPU and multicore CPU support enabled
        //            int _sliceCount = (int)Math.Round((double)movieFiles.Count / _CPUCount);
        //            for (int _i = 0; _i < _CPUCount; _i++)
        //            {
        //                List<FileInfo> _tmp = new List<FileInfo>();
        //                for (int _fi = (_i * _sliceCount); (_fi < ((_i + 1) * _sliceCount)) || ((_i == _CPUCount - 1) && (_fi < movieFiles.Count)); _fi++)
        //                {
        //                    if (_fi <= movieFiles.Count - 1)
        //                    {
        //                        try
        //                        {
        //                            _tmp.Add(movieFiles[_fi]);
        //                        }
        //                        catch { }
        //                    }
        //                }
        //                WorkerClass _wc = new WorkerClass();
        //                _wc.Templates = templates;
        //                _wc.GenerateFolderJpg = generateFolderJpg;
        //                _wc.GenerateThumbnail = generateThumb;
        //                _wc.UpdateIMDbRating = updateRating;
        //                _wc.MovieFiles = _tmp;

        //                Thread _thread = new Thread(new ParameterizedThreadStart(_wc.DoWork));
        //                _thread.IsBackground = true;
        //                _thread.SetApartmentState(ApartmentState.STA);
        //                _thread.Name = "Moviesheets update worker";
        //                _thread.Priority = ThreadPriority.BelowNormal;
        //                _handles[_i] = _wc.DoneEvent;
        //                _thread.Start();
        //            }
        ...
        //                }
        //                _b = true;
        //                foreach (WaitHandle _handle in _handles)
        //                {
        //                    bool _cb = _handle.WaitOne(50);
        //                    if (!_cb)
        //                    {
        ...
        //private static void ApplyNewTemplateEx(bool sync, IList<FileInfo> movieFiles, List<TemplateItem> templates, bool generateFolderJpg, bool generateThumb, bool updateRating)
        //{
        //    try
        //    {
        //        int _total = movieFiles.Count;
        ...
        //            if (sync)
        //            {
        //                FileManager.ShowAdorner(string.Format("Processing {0} ({1}/{2}) ...", _file.Name, _cnt, _total), true);
        //            }
        //            else
        //            {
        //                Loggy.Logger.Debug(string.Format("Processing {0} on thread {1} ", _file.Name, Thread.CurrentThread.ManagedThreadId));
        //            }
        //            MovieItem _movieItem = FileManager.GetMovieByFilePath(_file.FullName);
        //            FileManager.SetMovieItemStatus(_movieItem, MovieItemStatus.Querying);
        ...
        //            {
        //                // we have a candidate ;)
        //                MoviesheetsUpdateManager MetadataManager = MoviesheetsUpdateManager.CreateManagerForMovie(_file.FullName);
        //                try
        //                {
        //                    MoviesheetInfo _metadataInfo = MetadataManager.GetMetadataInfo();

        //                    // if required generate extra-thumbnail and/or thumbnail
        //                    string _ext = _metadataInfo != null && !string.IsNullOrEmpty(_metadataInfo.CoverExtension) ? _metadataInfo.CoverExtension : ".jpg";
        //                    string _tmpCoverPath = Helpers.GetUniqueFilename(_ext);
        //                    try
        //                    {
        //                        if (generateFolderJpg || generateThumb)
        //                        {
        //                            MetadataManager.GetImage(MoviesheetsUpdateManager.COVER_STREAM_NAME, _tmpCoverPath);
        //                            // set movie status to Done to indicate some processing was done
        //                            FileManager.SetMovieItemStatus(_movieItem, MovieItemStatus.Done);
        //                        }
        //                        if (generateFolderJpg)
        //                        {
        //                            string _folderJpgPath = FileManager.Configuration.GetFolderjpgPath(_file.FullName, true);
        //                            Helpers.CreateExtraThumbnailImage(_tmpCoverPath, _folderJpgPath);
        //                        }
        //                        if (generateThumb)
        //                        {
        //                            string _thumbPath = FileManager.Configuration.GetThumbnailPath(_file.FullName, true);
        //                            Helpers.CreateThumbnailImage(_tmpCoverPath, _thumbPath, FileManager.Configuration.Options.KeepAspectRatio);
        //                        }
        //                    }
        //                    finally
        //                    {
        //                        if (File.Exists(_tmpCoverPath))
        //                        {
        //                            File.Delete(_tmpCoverPath);
        //                        }
        //                    }

        //                    MovieSheetsGenerator _msGen = new MovieSheetsGenerator(SheetType.Main, _file.FullName);
        //                    if (templates.Count > 0)
        //                    {
        //                        _msGen.SelectedTemplate = templates[0]; // template for the mainsheet
        //                        MetadataManager.GetImage(MoviesheetsUpdateManager.COVER_STREAM_NAME, _msGen.CoverTempPath);
        //                        MetadataManager.GetImage(MoviesheetsUpdateManager.BACKGROUND_STREAM_NAME, _msGen.BackdropTempPath);
        //                        MetadataManager.GetImage(MoviesheetsUpdateManager.FANART1_STREAM_NAME, _msGen.Fanart1TempPath);
        //                        MetadataManager.GetImage(MoviesheetsUpdateManager.FANART2_STREAM_NAME, _msGen.Fanart2TempPath);
        //                        MetadataManager.GetImage(MoviesheetsUpdateManager.FANART3_STREAM_NAME, _msGen.Fanart3TempPath);
        //                        _msGen.MovieInfo = MetadataManager.GetMovieInfo();
        //                        if (_msGen.MovieInfo != null)
        //                        {
        //                            try
        //                            {
        //                                _msGen.MediaInfo = _msGen.MovieInfo.MediaInfo;
        //                            }
        //                            catch { }
        ...
        //                                try
        //                                {
        //                                    string _newRating = new IMDBMovieInfo().GetIMDbRating(_msGen.MovieInfo.IMDBID);
        //                                    if (!string.IsNullOrEmpty(_newRating))
        //                                    {
        //                                        _msGen.MovieInfo.Rating = _newRating;
        //                                    }
        //                                }
        //                                catch { }
        //                            }
        ...
        //                        MovieSheetsGenerator _extra = _msGen.Clone(false, SheetType.Extra);
        //                        if (templates.Count > 1)
        //                        {
        //                            _extra.SelectedTemplate = templates[1]; // template for the folder sheet
        //                        }
        //                        else
        //                        {
        //                            _extra.SelectedTemplate = templates[0];
        //                        }

        //                        MovieSheetsGenerator.RenderAndReplicateFinalMoviesheet(_msGen, _extra, null, true);

        //                        FileManager.SetMovieItemStatus(_movieItem, MovieItemStatus.Done);
        //                    }
        //                    else
        //                    {
        //                        // throw exception if current status is not Done (set maybe by create thumbnail/folderjpg)
        //                        if (_movieItem != null && _movieItem.MovieItemStatus != MovieItemStatus.Done)
        //                        {
        ...
        //                {
        //                    FileManager.SetMovieItemStatus(_movieItem, MovieItemStatus.Exception);
        //                    Loggy.Logger.DebugException("Cannot apply template: ", ex);
        //                }
        //            }
        //            else
        //            {
        ...
        //            if (!sync)
        //            {
        //                if (MoviesheetsUpdateManager.MovieDone != null)
        //                {
        //                    MoviesheetsUpdateManager.MovieDone(_movieItem, new MovieDoneEventArgs(_movieItem.Filename));
        //                }
        //            }
        ...
        //                try
        //                {
        //                    (Application.Current.MainWindow as ThumbGenMainWindow).progressListBox.ScrollIntoViewCentered(_movieItem);
        //                }
        //                catch { }
        //            }
        //            // take a breath
